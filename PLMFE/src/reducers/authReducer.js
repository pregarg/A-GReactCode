...
const authReducer= (state=INITIAL_STATE,action)=>{
   ...
   switch(action.type) {
       case SIGN_IN:
           console.log(action);
           if(action.payload&&action.payload.status&&action.payload.status<400){
               ...
               if(!action.payload.data.loginOutput.foundError){
                   return {
                       ...
                       message: "message",
                       expiry: getExpiry(action.payload.data.loginOutput.participant.token),
           ...
           else if(action.payload&&action.payload.status&&action.payload.status>399){
               return {...state, isSignedIn:false, userId: null, token:null, message:""};
           }
           //break;
       case SIGN_OUT:
           if(state.isSignedIn){
