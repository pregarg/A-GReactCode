...
export default function AdvancedSearchModal(prop) {
      ...
      if (mastersSelector["masterTypeCommon"].length > 0) {
        ...
        mastersSelector["masterTypeCommon"].forEach((elem) => {
          if (elem.Type.toLowerCase() === "transactiontype")
            newstateOptions.push({
              label: convertToCase(elem.DisplayName),
              value: convertToCase(elem.ActualName),
            });
        });
  ...
  const handleModalShowHide = (flag) => {
    printConsole(
      "Inside AdvancedSearch handleModalShowHide state value: ",
      fieldState,
    );
    printConsole(
      "Inside AdvancedSearch handleModalShowHide flag value: ",
      flag,
    );
  ...
  const setSelectStateData = (selectedValue, evnt) => {
    ...
    console.log(
      "Inside setSelectStateData with selectedValue: ",
      selectedValue,
    );
    const { name } = evnt;
        ...
        if (mastersSelector["masterStageTable"][0].length > 0) {
          ...
          mastersSelector["masterStageTable"][0].forEach((elem) => {
            if (elem.FlowId === flowId)
              newstateOptions.push({
                label: convertToCase(elem.StageName),
                value: convertToCase(elem.StageName),
              });
          });
          ...
          <div className="col-xs-6 col-md-6">
            <div className="form-floating">
              ...
              <Select
                ...
                className="basic-multi-select"
                options={[
                  { label: "SELFSERVICE", value: 2 },
                  { label: "CONTRACTING", value: 1 },
                ]}
          ...
          <div className="col-xs-6 col-md-6">
            <div className="form-floating">
              ...
              <Select
                ...
                id="stageDropdown"
                value={{
                  label: fieldState.StageName,
                  value: fieldState.StageName,
                }}
                isMulti={false}
                //onChange={(selectValue,event)=>(handleLinearSelectChange(selectValue, event))}
                onChange={(selectValue, event) => {
                ...
                //styles={{...customStyles}}
                isDisabled={
                  selectValues.hasOwnProperty("stageDropDown") ? false : true
                }
                isSearchable={
                  document.documentElement.clientHeight >
                  document.documentElement.clientWidth
  ...
  return (
    <>
      ...
      >
        ...
        <Modal.Body>{searchFields()}</Modal.Body>
        <Modal.Footer>
          <button
            type="button"
