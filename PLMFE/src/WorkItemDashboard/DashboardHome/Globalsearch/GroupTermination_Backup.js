...
export default function GroupTermination() {
    ...
    const stateOptions = ((mastersSelector['masterStateSymbol'].length === 0) ? [] : (mastersSelector['masterStateSymbol'][0].data));
    ...
    const handleCheckBoxChange = (evnt, ind) => {
        ...
        let jsn = selectedType == 'Provider' ? providerTableData[ind] : facilityAncillaryTableData[ind];
...
//Till Here
    ...
    const showValues = (data, type) => {
        ...
        if(data?.SOURCE=='MultiPlan')
        {
              ...
              .catch((err) => {
                console.log("Caught in generic get api for MultiPlan ViewData: ",err.message);
             });
        }
        else
        {
    ...
    const validateState = () => {
        ...
        if (formData["state"].value!=undefined && formData["state"].value !== "") {
            ...
            if(validateStateFlag < 2){
                alert("Please select atleast one more field other than State to do Search.")
                return false;
            }
        ...
        if ( validateState() && isFormValid) {
            ...
            axios.post(baseURL + '/generic/callProcedure', getApiJson, { headers: { 'Authorization': `Bearer ${token}` } }).then((res) => {
                ...
                if (selectedType == 'Provider') {
                    ...
                    (data.length === 0 || data === undefined) ? providerDataArray = [] : data.map(arr => {
                        let obj = {};
                        let finalObj=[];
                        ...
                        value['VIEWDATA'] = <div
                        onClick={(e) => { e.preventDefault(); showValues(value, selectedType) }}
                    ...
                    console.log("Response providerDataArray",providerDataArray[0]);
                ...
                else
                {
                    ...
                    (data.length === 0 || data === undefined) ? facAncDataArray = [] : data.map(arr => {
                        let obj = {};
                        let finalObj=[];
                        arr.forEach((value, index) => {
            ...
            .catch((err) => {
                console.log("Error in calling callProcedure option GETPROVIDERSEARCHDATA: ", err.message);
            });
            setVisible(true);
        }
    ...
    const renderForm = (selectedType) => {
        return <form onSubmit={newSearchHandler} noValidate>
            {(selectedType == "Provider") &&
                ...
                <div className="col-xs-6 col-md-6">
                        ...
                        <input type="text" className={formData.firstName.isInvalid ? 'form-control is-invalid' : 'form-control'} id="firstName" placeholder="firstName" value={formData.firstName.value} name="firstName" onChange={(e) => updateFormData(e.target.value, 'firstName')} ></input>
                    ...
                    <div className="col-xs-6 col-md-6">
                        <div className="form-floating">
                        <input type="text" className={formData.lastName.isInvalid ? 'form-control is-invalid' : 'form-control'} id="lastName" placeholder="lastName" value={formData.lastName.value} name="lastName" onChange={(e) => updateFormData(e.target.value, 'lastName')} ></input>
            ...
            <div className="row my-2">
                ...
                <div className="col-xs-6 col-md-6">
                    <div className="form-floating">
                        <input type="text" className={formData.payToNPI.isInvalid ? 'form-control is-invalid' : 'form-control'} id="payToNPI" placeholder="payToNPI" value={formData.payToNPI.value} name="payToNPI" onChange={(e) => updateFormData(e.target.value, 'payToNPI')} ></input>
                        <label htmlFor="floatingInputGrid">Pay To NPI</label>
                        {formData.payToNPI.isInvalid && <div className="small text-danger">Pay To NPI is required</div>}
                    </div>
                </div>
                     )
                    }
            ...
            {(selectedType == "Provider") &&
                (
                 <div className="col-xs-6 col-md-6">
                            <div className="form-floating">
                                <input type="text" className={formData.organizationName.isInvalid ? 'form-control is-invalid' : 'form-control'} id="organizationName" placeholder="organizationName" value={formData.organizationName.value} name="organizationName" maxLength="100" onChange={(e) => updateFormData(e.target.value, 'organizationName')} />
                                <label htmlFor="floatingInputGrid">Legal Entity Name</label>
                                {formData.organizationName.isInvalid && <div className="small text-danger">Organization Name is required</div>}
                            </div>
                        </div>
                )
            }
                {(selectedType == 'Facility' || selectedType == 'Ancillary') &&
                    (
                        ...
                        <div className="col-xs-6 col-md-6">
                            <div className="form-floating">
                                <input type="text" className={formData.legalEntityName.isInvalid ? 'form-control is-invalid' : 'form-control'} id="legalEntityName" placeholder="legalEntityName" value={formData.legalEntityName.value} name="legalEntityName" onChange={(e) => updateFormData(e.target.value, 'legalEntityName')} ></input>
                                <label htmlFor="floatingInputGrid">Legal Entity Name</label>
                                {formData.legalEntityName.isInvalid && <div className="small text-danger">Legal Entity Name is required</div>}
                            </div>
                        </div>

                        <div className="col-xs-6 col-md-6">
                            <div className="form-floating">
                                <input type="text" className={formData.dbaName.isInvalid ? 'form-control is-invalid' : 'form-control'} id="dbaName" placeholder="dbaName" value={formData.dbaName.value} name="dbaName" onChange={(e) => updateFormData(e.target.value, 'dbaName')} ></input>
                                <label htmlFor="floatingInputGrid">DBA Name</label>
                                {formData.dbaName.isInvalid && <div className="small text-danger">DBA Name is required</div>}
                            </div>
                        </div>
                        </>
                    )
            ...
            {isSearchClicked && !isFormValid && (
                <div className="row my-2">
                    <div className="col-xs-12">
                        <div className="small text-danger">Atleast one entry is required</div>
                    </div>
                </div>
            )}
            <div className="row my-2">
                <div className="col-xs-12 gap-2 col-md-12 d-flex justify-content-end">
                    ...
                    <button onClick={clearFields} className="btn btn-outline-primary btnStyle">Clear</button>
                </div>
            </div>
            ...
            {(visible && !isProvider) &&
                (<div div className="row my-2" >
                    <div className="row">
                        <div className="col-xs-6">
                            <TableComponent columnName={facilityAncillaryColumnNames}
    ...
    return (
        <>
            <div className="GroupTermination backgroundColor" style={{  minHeight: "100%" }}>
                <div className="container">
                    <div className="row">
                        <div className="col-xs-6" style={{ textAlign: "center" }}>
                            <br />
                            <label id="tileFormLabel" className='HeadingStyle'>Search</label>
                        </div>
                    </div>
                </div>
                <br />
                <div className="container" style={{ overflow: "auto", height: "auto", minHeight: "86vh" ,minWidth: "98%"}}>
                    <div className="accordion AddProviderLabel" id="accordionPanelsStayOpenExample">
                        <div className="accordion-item">
                            ...
                            <div id="panelsStayOpen-collapseOne" className="accordion-collapse collapse show" aria-labelledby="panelsStayOpen-headingOne">
                                <div className="accordion-body">
                                    <div className="row my-2">
                                        <div className="col-xs-12">
                                            <label id="instructionHeading" className='instructionHeading'>
                                                The below search screen is defined by Provider, Ancillary and Facility. Please enter one field from the below information and the data of provider(s) associated will populate on the screen. Once the search is complete, you may click on the provider to obtain the information.
                                                Search example: St. Joseph – this will return multiple records with St. Joseph. Type in St. Joseph’s Hospital with NPI, the specified Facility will populate with this data on the screen.</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="accordion-item">
                            <h2 className="accordion-header" id="panelsStayOpen-headingTwo">
                                <button className="accordion-button accordionButtonStyle" type="button" data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapseTwo" aria-expanded="false" aria-controls="panelsStayOpen-collapseTwo">
                                    Type
                                </button>
                            </h2>
                                ...
                                <div className="accordion-body">
                                    <div className="row g-2">
                                        <div className="col-xs-6 col-md-6">
                                            ...
                                            {typeFieldError && <div className='text-danger small'>This field is required</div>}
                                        </div>
                        ...
                        {(selectedType == "Provider" || selectedType == 'Facility' || selectedType == 'Ancillary') &&
                (
                        <div className="accordion-item">
            ...
            >
                ...
                {modalData?.SOURCE=='PDM'?
                    <Globalsearch
                        ...
                        caseNumber={(modalData.CaseNumber === null) ? '':modalData.CaseNumber}
                    ...
                    />:<></>}
                                  ...
                                  <div className="GroupTermination backgroundColor" style={{  minHeight: "100%" }}>
                                   <div className="cardsContainer" style={{ width: '99%' }}>
                                    <div className="cardsContainerChild" style={{ width: '99%' }}>
                                    <div className="card" style={{ border: '5px solid rgb(101, 101, 233)' , minHeight: 350 }}>
                                    <div className="col-xs-6" id="caseTable1">

                                             <p style={{ textAlign: "center", fontSize: "var(--font-size-large-1)", fontWeight: "bold" }}>
                                               MultiPlan Data
                                               <button className="btn" onClick={event =>closeModal()} style={{ float: "right"}}>
                                               <i className="fa fa-close"></i>
                                               </button></p>
                                    </div>
                                           <div className="card-body">
                                            <div className="card-text my-2" style={{ 
                                                maxHeight: "830px", 
                                                overflow: "auto" 
                                                }}>
