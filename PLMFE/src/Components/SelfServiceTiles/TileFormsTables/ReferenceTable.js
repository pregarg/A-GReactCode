...
export default function RefereceTable ({referenceTableRowsData}){
    ...
    const tdData = () => {
        ...
        return referenceTableRowsData.map((data, index)=>{
            ...
            const reference = (('reference' in data) && (data.reference.value !== undefined)) ? (data.reference.value) : (data.reference);
            return(
                <tr key={index}>
                
                    <td>
                        {(('name' in data) && (data.name.value !== undefined)) ? (data.name.value) : (data.name)}
                    </td>
                    <td>
