...
export default function FIRLTable({
  firlTableRowsData,
  deleteTableRows,
            ...
            <div className="col-xs-6 col-md-3">
              ...
              <input
                ...
                value={
                  data &&
                  "providerNo" in data &&
                  data.providerNo.value !== undefined
                    ? convertToCase(data.providerNo.value)
                    : convertToCase(data.providerNo)
                }
                //onBlur={()=>{ if(!isTouched.providerNo){setIsTouched({...isTouched, providerNo: true})}}}
                onChange={(evnt) => {
                  handleGridFieldChange(index, evnt, FIRLTable.displayName);
                  setIsTouched({ ...isTouched, providerNo: evnt.target.value });
                }}
                name="providerNo"
                className={`form-control ${(!data.providerNo || (data.providerNo && !data.providerNo.value)) && isTouched.providerNo === true ? "is-invalid" : ""}`}
                title="Please Enter Valid Provider#"
            ...
            <div className="col-xs-6 col-md-3">
              ...
              <input
                ...
                value={
                  data &&
                  "component" in data &&
                  data.component.value !== undefined
                    ? convertToCase(data.component.value)
                    : convertToCase(data.component)
                }
                //onBlur={()=>{ if(!isTouched.component){setIsTouched({...isTouched, component: true})}}}
                onChange={(evnt) => {
                  handleGridFieldChange(index, evnt, FIRLTable.displayName);
                  setIsTouched({ ...isTouched, component: evnt.target.value });
                }}
                name="component"
                className={`form-control ${(!data.component || (data.component && !data.component.value)) && isTouched.component === true ? "is-invalid" : ""}`}
                title="Please Enter Valid Component"
                disabled={lockStatus !== "N"}
              />
            </div>
            <div className="col-xs-6 col-md-3">
              ...
              <input
                ...
                value={
                  data &&
                  "currentRate" in data &&
                  data.currentRate.value !== undefined
                    ? convertToCase(data.currentRate.value)
                    : convertToCase(data.currentRate)
                }
                ...
                onChange={(evnt) => {
                  ...
                  setIsTouched({
                    ...isTouched,
                    currentRate: evnt.target.value,
                  });
                }}
                id="currentRate"
                //  placeholder="9xx-xx-xxxx" pattern="[9]{1}[0-9]{2}-[0-9]{2}-[0-9]{4}"
                name="currentRate"
                className={`form-control ${(!data.currentRate || (data.currentRate && !data.currentRate.value)) && isTouched.currentRate === true ? "is-invalid" : ""}`}
          ...
          <div className="row">
            <div className="col-xs-6 col-md-3">
              ...
              <input
                ...
                type="text"
                value={
                  data && "newRate" in data && data.newRate.value !== undefined
                    ? convertToCase(data.newRate.value)
                    : convertToCase(data.newRate)
                }
                //onBlur={()=>{ if(!isTouched.newRate){setIsTouched({...isTouched, newRate: true})}}}
                onChange={(evnt) => {
                  handleGridFieldChange(index, evnt, FIRLTable.displayName);
                  setIsTouched({ ...isTouched, newRate: evnt.target.value });
                }}
                id="newRate"
                name="newRate"
                className={`form-control ${(!data.newRate || (data.newRate && !data.newRate.value)) && isTouched.newRate === true ? "is-invalid" : ""}`}
            ...
            <div className="col-xs-6 col-md-3">
              ...
              <input
                ...
                type="text"
                value={
                  data && "comment" in data && data.comment.value !== undefined
                    ? convertToCase(data.comment.value)
                    : convertToCase(data.comment)
                }
                //onBlur={()=>{ if(!isTouched.comment){setIsTouched({...isTouched, comment: true})}}}
                onChange={(evnt) => {
                  handleGridFieldChange(index, evnt, FIRLTable.displayName);
                  setIsTouched({ ...isTouched, comment: evnt.target.value });
                }}
                id="comment"
                name="comment"
                className={`form-control ${(!data.comment || (data.comment && !data.comment.value)) && isTouched.comment === true ? "is-invalid" : ""}`}
      ...
      return firlTableRowsData.map((data, index) => {
        ...
        return (
          <tr key={index}>
              ...
              <td>
                ...
                >
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      editTableRows(index, FIRLTable.displayName);
            ...
            <td>
              {"providerNo" in data && data.providerNo.value !== undefined
                ? convertToCase(data.providerNo.value)
                : convertToCase(data.providerNo)}
            </td>
            <td>
              {"component" in data && data.component.value !== undefined
                ? convertToCase(data.component.value)
                : convertToCase(data.component)}
            </td>
            <td>
              {"currentRate" in data && data.currentRate.value !== undefined
                ? convertToCase(data.currentRate.value)
                : convertToCase(data.currentRate)}
            </td>
            <td>
              {"newRate" in data && data.newRate.value !== undefined
                ? convertToCase(data.newRate.value)
                : convertToCase(data.newRate)}
            </td>
            <td>
              {"comment" in data && data.comment.value !== undefined
                ? convertToCase(data.comment.value)
                : convertToCase(data.comment)}
            </td>
          </tr>
  ...
  return (
    <>
            ...
            {lockStatus == "N" ? (
              <th scope="col" style={{ width: "7.5%" }}>
                <button
                  className="addBtn"
                  onClick={() => {
                    addTableRows(FIRLTable.displayName);
                    handleModalChange(true);
                    handleDataIndex(firlTableRowsData.length);
      ...
      {firlTableRowsData !== undefined && firlTableRowsData.length <= 0 ? (
        <div className="invalid-feedback" style={{ display: "block" }}>
          Atleast one entry is required if Critical Access Hospital is 'Yes'
        </div>
      ) : null}
      <GridModal
        name={"Hospital Compensation Details"}
        ...
        gridName={FIRLTable.displayName}
