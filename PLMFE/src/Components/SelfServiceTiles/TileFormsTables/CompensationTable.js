...
export default function CompensationTable({
          ...
          <div className="row">
            <div className="col-xs-6 col-md-3">
              ...
              <input
                ...
                type="text"
                value={
                  data && "rate" in data && data.rate.value !== undefined
                    ? convertToCase(data.rate.value)
                    : convertToCase(data.rate)
                }
                onChange={(evnt) => {
                  handleGridFieldChange(
                    index,
                    evnt,
                    CompensationTable.displayName,
                  );
                }}
                name="rate"
  ...
  const tdData = () => {
    ...
    if (
      ...
      compensationTableRowsData.length > 0
    ) {
      ...
      return compensationTableRowsData.map((data, index) => {
        ...
        return (
          <tr key={index}>
              ...
              <td>
                ...
                >
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      editTableRows(index, CompensationTable.displayName);
            ...
            <td>
              {"rate" in data && data.rate.value !== undefined
                ? convertToCase(data.rate.value)
                : convertToCase(data.rate)}
            </td>
            <td>
              {"schedule" in data && data.schedule.value !== undefined
                ? convertToCase(data.schedule.value)
                : convertToCase(data.schedule)}
            </td>
  ...
  return (
    <>
            ...
            {lockStatus == "N" ? (
              <th scope="col" style={{ width: "7.5%" }}>
                <button
                  className="addBtn"
                  onClick={() => {
                    addTableRows(CompensationTable.displayName);
                    handleModalChange(true);
                    handleDataIndex(compensationTableRowsData.length);
      ...
      <GridModal
        name={"Provider Compensation Details"}
        ...
        gridName={CompensationTable.displayName}
