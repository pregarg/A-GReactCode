...
const ExpeditedRequestAccordion = (props) => {
  ...
  const [expeditedRequestData, setExpeditedRequestData] = useState(
    props.expeditedRequestData,
  );

  const expReqSelector = useSelector(
    (state) => state?.masterAngExpeditedRequested,
  );
  const expDenSelector = useSelector(
    (state) => state?.masterAngExpeditedDenied,
  );
        ...
        {({ meta }) => (
          <div className="form-floating">
            <Select
              ...
              isMulti={false}
              onChange={(value) =>
                handleExpeditedRequestData(name, value?.value, true)
              }
              value={
                expeditedRequestData[name]
                  ? {
  ...
  const DatePicker = (name, label, placeholder) => {
    ...
    const dateValue = expeditedRequestData[name + "#date"]
      ? new Date(expeditedRequestData[name + "#date"])
      : expeditedRequestData[name];
    return (
      <div>
        <ReactDatePicker
  ...
  useEffect(() => {
    ...
    setExpeditedRequestedValues(
      expReq.map((e) => e.Expedited_Requested).map(kvMapper),
    );

    const expDen = expDenSelector?.[0] || [];
    setExpeditedDeniedValues(
      expDen.map((e) => e.Expedited_Denied).map(kvMapper),
    );
            ...
            <div className="row my-2">
              ...
              <div className="col-xs-6 col-md-4">
                {SelectField(
                  ...
                  "Standard Upgraded to Expedited",
                  stUpExpeditedValues,
                )}
              </div>
            </div>
            <div className="row my-2">
              ...
              <div className="col-xs-6 col-md-4">
                {SelectField(
                  "Expedited_Denied",
                  "Expedited Denied",
                  expeditedDeniedValues,
                )}
              </div>
              <div className="col-xs-6 col-md-4">
                {DatePicker(
                  "Expedited_Denied_Date",
                  "Expedited Denied Date",
                  "Expedited Denied Date",
                )}
              </div>
            </div>
