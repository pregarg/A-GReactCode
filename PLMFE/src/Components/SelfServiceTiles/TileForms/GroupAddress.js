...
export default function GroupAddress() {
    ...
    useEffect(() => {
         if(prop.state !== null){
            ...
            let getCaseNumberEndpoints = [
              ...
              axios.all(getCaseNumberEndpoints.map((endpoint) => axios.get(endpoint))).then((res) => {
                 if(res[0].status === 200){
                    //apiArray = [...res[3].data];
       ...
       const modifyValidatedAddressRow = (index,validateLocationState) => {
            ...
            rowsInput[index]['stateValue'] = {label:validateLocationState['state'], value: validateLocationState['state']};
  ...
  return (
    <>
    <div className="GroupAdddress backgroundColor" style={{minHeight:"100vh"}}>
     <div className="container">
            <div className="row">
                <div className="col-xs-6" style={{textAlign:"center"}}>
                    <br />
                      <label id="tileFormLabel" className='HeadingStyle'>Group Address Modification</label>
                      ...
                      <button type="button" className="btn btn-outline-primary btnStyle" onClick={event => saveGridData(event)} style={{float:"right",marginRight:"10px"}}>Save</button>
                 </div>
            </div>
        </div>
        <br/>
                                    ...
                                    <div className="accordion-body">
                                        <div className="row">
                                            <div className="col-xs-6 col-md-12">
                                                ...
                                                handleGridFieldChange={handleGridFieldChange} modifyValidatedAddressRow={modifyValidatedAddressRow}
                                                calledFormName={GroupAddress.displayName}></LocationTable>
                                            </div>
                                        </div>
                                    </div>
                                </div>
