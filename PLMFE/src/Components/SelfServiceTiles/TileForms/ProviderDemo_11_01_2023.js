...
export default function ProviderDemo() {
  ...
  const providerColumnNames =
    "Organization Name~name, Provider ID~PROVIDERID, Contract ID~CONTRACTID, First Name~FIRSTNAME, Last Name~LASTNAME, NPI ID~NPIID, Provider Status~PROVIDERSTATUS";
  ...
  useEffect(() => {
    ...
    //Added by Nidhi
    selectJson["additionalQues"]
      .filter((data) => data.transactionType == ProviderDemo.displayName)
      .map((val) =>
  ...
  const saveFormData = (values) => {
    try {
      ...
      if (checkIfValueExistsInState()) {
        ...
        requestBody.gender =
          apiTestState.gender !== undefined ? apiTestState.gender.value : "";
    ...
    try {
      ...
      if (validated || callProcRef.current !== "callProc") {
        ...
        if (!isDecisionDiscard) {
          if (
            callProcRef.current === "callProc" &&
            gridDataRef.current.SelfServ_Prov_Details.ecfmgQues === "Yes"
          ) {
            if (
              gridDataRef.current.SelfServ_Prov_Details.ecfmgNumber === "" ||
              gridDataRef.current.SelfServ_Prov_Details.ecfmgNumber ===
                undefined ||
              gridDataRef.current.SelfServ_Prov_Details.ecfmgIssueDate === "" ||
              gridDataRef.current.SelfServ_Prov_Details.ecfmgIssueDate ===
                undefined
            ) {
  ...
  const callGenericGet = () => {
    ...
    if (caseUnlockState !== -1) {
      ...
      if (
        ...
        numberOfChecks.PROVIDERSTATUS == "Terminated"
      ) {
        alert(
                        ...
                        <div className="accordion-body">
                          <div className="row">
                            <div className="col-xs-12">
                              ...
                              >
                                To modify Provider Address, please provide
                                Organization Name and select Provider Address
                                  ...
                                  {({
                                    ...
                                    <div className="form-floating">
                                      <input
                                        maxLength="100"
                                        type="text"
                                        className={`form-control ${meta.touched && meta.error ? " is-invalid" : field.value || field.value === null ? "is-valid" : ""}`}
                                        placeholder="John"
                                        {...field}
                                        disabled
                                      />
