...
const populateAccessibility = (node, staticValue)=>{
    return !!checkDataAvailable(node.Accessibility) ?
        (Array.isArray(node.Accessibility) ?
            ...
            : ((!!checkDataAvailable(node.Accessibility.Accessibility) && checkDataAvailable(node.Accessibility.Accessibility.AccessibilityDescription) == staticValue
            && checkDataAvailable(node.Accessibility.AccessibilityFlag)=='1') ? {label:'Yes',value: 'Y'} : {label: 'No',value: 'N'}))
        : {label: 'No',value: 'N'}
}
...
export default function ProviderDemo() {
   ...
   //ProviderDemo.displayName = "Address Modification";
    ...
    const masterUserName = ((mastersSelector.hasOwnProperty('auth')?(mastersSelector.auth.hasOwnProperty('userName')?mastersSelector.auth.userName:'system'):'system'));
    ...
    let credentialingConfigData = JSON.parse(process.env.REACT_APP_CREDENTIALING_DETAILS);
    const dispatch=useDispatch();
    const [loadForm, setLoadForm] = useState(true);
    const [loading, setLoading] = useState(false);
    const [caqhModal, setCaqhModal] = useState({id:null, header: null, body: null, show: false});
    const [caqhGenericModal, setCaqhGenericModal] = useState({header: null, body: null, state: false});
    ...
    const validationSchema = Yup.object().shape({
        ...
        //          .max(100,"Organization Name max length exceeded"),
    ...
    const providerColumnNames = 'Organization Name~name, Provider ID~PROVIDERID, Contract ID~CONTRACTID, First Name~FIRSTNAME, Last Name~LASTNAME, NPI ID~NPIID, Provider Status~PROVIDERSTATUS';
   ...
   /* const handleCheckBoxChange = (evnt,ind) => {
    ...
    const falseExtraChecked = () => {
        let retJsn = providerTableData.filter((el) => {el.isChecked=false});
        return retJsn;
    }

    const clearDataOnCheckBoxChange=()=>{
        setApiTestState({
            AddModification: apiTestState.AddModification!==undefined && apiTestState.AddModification==true?true:false,
            PayToModification: apiTestState.PayToModification!==undefined && apiTestState.PayToModification==true?true:false,
           organizationName:'',
           firstName:'',
           middleName: '',
           lastName: '',
    ...
    const handleCheckBoxChange = (evnt, ind) => {
        ...
        console.log('Inside handlecheckbox change with event: ',evnt.target.checked);
        console.log('Inside handlecheckbox change with index: ',ind,' and value: ',isChecked);
        printConsole('Inside handleCheckBox change caseUnlockState value: ',caseUnlockState);
        if(isChecked){
            if(caseUnlockState !== -1){
                ...
                printConsole("Inside handleCheckBox change else with updated data: ",updatedProvData);
    ...
    const showGridCheckbox = ((mastersSelector.hasOwnProperty('auth') && (mastersSelector.auth.hasOwnProperty('userType')) && mastersSelector.auth.userType === 'A'))?true:true;
    //Till Here

    //const {validate,errors,handleSubmit} = useValidateForm();

    const {submitCase,updateLockStatus, validateGridData, printConsole, setContractIdDropDown, updateDecision,checkDecision} = useUpdateDecision();
 ...
 //Added Newly by Nidhi Gupta on 6/2/2023
    ...
    const checkLuhn=(caqhNpiId)=>
    {
        ...
        if (caqhNpiId.length > 0)
        {
            ...
            var j;
            i = caqhNpiId.length;
            if ((i == 15) && (caqhNpiId.indexOf("80840", 0, 5) == 0))
                sum = 0;
            else if (i == 10)
                sum = 24;
            else

            return("This is not a valid NPI ID");
    ...
    useEffect(() => {
        ...
        if(mastersSelector.hasOwnProperty('masterAgesSeen')){
            ...
            let orgAgesSeenArray = ((mastersSelector['masterAgesSeen'].length===0) ? [] : (mastersSelector['masterAgesSeen'][0].data));
            for (let i=0; i<orgAgesSeenArray.length; i++)
            {
                newAgesSeenArray.push({'label':orgAgesSeenArray[i],'value':orgAgesSeenArray[i]});
            }
         ...
         if(mastersSelector.hasOwnProperty('masterLanguages')){
            ...
            selectJson.languageArray = ((mastersSelector['masterLanguages'].length===0) ? [] : (mastersSelector['masterLanguages'][0].data));
         }

         if(mastersSelector.hasOwnProperty('masterGridLicenseType')){
            selectJson.licenseTypeOptions = ((mastersSelector['masterGridLicenseType'].length===0) ? [] : (mastersSelector['masterGridLicenseType'][0].data));
         }
        ...
        //  }
         ...
         if(mastersSelector.hasOwnProperty('masterStateSymbol')){
            let newstateOptions=[];
            let orgstateOptions = ((mastersSelector['masterStateSymbol'].length===0) ? [] : (mastersSelector['masterStateSymbol'][0].data));
            for (let i=0; i<orgstateOptions.length; i++)
            {
                newstateOptions.push({'label':orgstateOptions[i],'value':orgstateOptions[i]});
            }
         ...
         if(mastersSelector.hasOwnProperty('masterAddressType')){
            selectJson.addressTypeOptions = ((mastersSelector['masterAddressType'].length===0) ? [] : (mastersSelector['masterAddressType'][0].data));
         }

         if(mastersSelector.hasOwnProperty('masterLicenseType')){
            selectJson.typeOptions = ((mastersSelector['masterLicenseType'].length===0) ? [] : (mastersSelector['masterLicenseType'][0].data));
         }

         if(mastersSelector.hasOwnProperty('masterSpeciality')){
            selectJson.specialtyOptions = ((mastersSelector['masterSpeciality'].length===0) ? [] : (mastersSelector['masterSpeciality'][0].data));
         }

         if(mastersSelector.hasOwnProperty('masterGraduateType')){
            selectJson.degreeOptions = ((mastersSelector['masterGraduateType'].length===0) ? [] : (mastersSelector['masterGraduateType'][0].data));
         }

         if(mastersSelector.hasOwnProperty('masterDocumentList')){
            selectJson.documentOptions = ((mastersSelector['masterDocumentList'].length===0) ? [] : (mastersSelector['masterDocumentList'][0].data));
         }
         if(mastersSelector.hasOwnProperty('masterAdditionalQues')){
            selectJson.additionalQues = ((mastersSelector['masterAdditionalQues'].length===0) ? [] : (mastersSelector['masterAdditionalQues'][0].data));
         }
         if(mastersSelector.hasOwnProperty('masterTaxonomyCode')){
            selectJson.taxonomyOptions = ((mastersSelector['masterTaxonomyCode'].length===0) ? [] : (mastersSelector['masterTaxonomyCode'][0].data));
         }
        ...
        {
        provModData.map(ele => { const org ={ label: ele.ORGANIZATIONNAME, value: ele.ORGANIZATIONNAME  } ;
            newOrg.push(org);
            });
    ...
    //Added by Nidhi
    selectJson['additionalQues'].filter(data => data.transactionType == ProviderDemo.displayName).map
    (val => additionalQuesValues.push({ questionId: val.questionId, label: val.quesDescription }));
    ...
    //till here 01
        ...
        if (prop.state.formView !== undefined && prop.state.formView === 'DashboardView'){
            ...
            // );
              ...
              let getApiJson= {};
              getApiJson['tableNames'] = getTableDetails()['providerLinear'].concat(getTableDetails()['gridTables']);
                ...
                if(apiStat === 0){
                    ...
                    respKeys.forEach(k => {
                            ...
                            if(respData[k][0]!==undefined){
                                ...
                                if(apiResponse.hasOwnProperty('ecfmgIssueDate')){
                                    if(typeof apiResponse.ecfmgIssueDate === 'string'){
                                        const eid = new Date(apiResponse.ecfmgIssueDate);
                                ...
                                if(apiResponse.hasOwnProperty('ecfmgExpirationDate')){
                                    if(typeof apiResponse.ecfmgExpirationDate === 'string'){
                                        const eed = new Date(apiResponse.ecfmgExpirationDate);
                                ...
                                if(apiResponse.hasOwnProperty('attestationDate')){
                                    if(typeof apiResponse.attestationDate === 'string'){
                                        const atd = new Date(apiResponse.attestationDate);
                                ...
                                apiResponse.gender = {'label':apiResponse.gender,'value':apiResponse.gender};
                                apiResponse.newPatients = {'label':apiResponse.newPatients,'value':apiResponse.newPatients};
                                apiResponse.agesSeen = {'label':apiResponse.agesSeen,'value':apiResponse.agesSeen};
                                apiResponse.placeInDirectory = {'label':apiResponse.placeInDirectory,'value':apiResponse.placeInDirectory};
                                apiResponse.delegated = {'label':apiResponse.delegated,'value':apiResponse.delegated};
                                apiResponse.ecfmgQues = {'label':apiResponse.ecfmgQues,'value':apiResponse.ecfmgQues};
                                apiResponse.contractId = {'label':apiResponse.contractId,'value':apiResponse.contractId};
                        ...
                        if(k === 'locationTable'){
                            ...
                            respData[k].forEach((js) => {
                                ...
                                if (apiResponse.hasOwnProperty('electronicHealthRecord')) {
                                    ...
                                    {apiResponse.electronicHealthRecord = {'label':'Yes','value':apiResponse.electronicHealthRecord};}
                                    else if  ( apiResponse.electronicHealthRecord === 'N')
                                    { apiResponse.electronicHealthRecord = {'label':'No','value':apiResponse.electronicHealthRecord};}
                                }
                                if (apiResponse.hasOwnProperty('publicTransportation')) {
                                    if ( apiResponse.publicTransportation === 'Y')
                                    {apiResponse.publicTransportation = {'label':'Yes','value':apiResponse.publicTransportation};}
                                    else if ( apiResponse.publicTransportation === 'N')
                                    { apiResponse.publicTransportation = {'label':'No','value':apiResponse.publicTransportation};}
                                }
                                if (apiResponse.hasOwnProperty('handicapAccess')) {
                                    if ( apiResponse.handicapAccess === 'Y')
                                    {apiResponse.handicapAccess = {'label':'Yes','value':apiResponse.handicapAccess};}
                                    else if ( apiResponse.handicapAccess === 'N')
                                    { apiResponse.handicapAccess = {'label':'No','value':apiResponse.handicapAccess};}
                                }
                                if (apiResponse.hasOwnProperty('tddHearing')) {
                                    if ( apiResponse.tddHearing === 'Y')
                                    {apiResponse.tddHearing = {'label':'Yes','value':apiResponse.tddHearing};}
                                    else if ( apiResponse.tddHearing === 'N')
                                    { apiResponse.tddHearing = {'label':'No','value':apiResponse.tddHearing};}
                                }
                        ...
                        if(k === 'payToTable'){
                            ...
                            respData[k].forEach((js) => {
                            const newJson = convertToDateObj(js);
                            console.log("Provider Modification payToTable newJson;", newJson);
   ...
   const handleGridSelectChange = (index,selectedValue,evnt,triggeredFormName) => {
    ...
    const checkIfValueExistsInState = () => {
        let retVal = false;
        printConsole("Inside checkIfValueExistsInState apiTestState: ",apiTestState);
        if(
            (apiTestState.hasOwnProperty('organizationName') && apiTestState.organizationName!==undefined && apiTestState.organizationName.length>0)
    ...
    const saveFormData = (values) => {
        ...
        console.log("Inside saveFormData()");
            ...
            if(checkIfValueExistsInState()){
                ...
                requestBody.userName = ((mastersSelector.hasOwnProperty('auth')?(mastersSelector.auth.hasOwnProperty('userName')?mastersSelector.auth.userName:'system'):'system'));
                ...
                console.log("Inside add provider create case master selector: ",mastersSelector);
                console.log("Inside add provider create case username: ",((mastersSelector.hasOwnProperty('auth')?(mastersSelector.auth.hasOwnProperty('userName')?mastersSelector.auth.userName:'system'):'system')));
                mainWIObject.createdByName = ((mastersSelector.hasOwnProperty('auth')?(mastersSelector.auth.hasOwnProperty('userName')?mastersSelector.auth.userName:'system'):'system'));
        ...
        if(isSearchClicked)
        {
            customAxios.post("/generic/create",apiJson,{headers:{'Authorization':`Bearer ${token}`}})
            .then((res) => {
                    ...
                    if(apiStat === 0){
                        ...
                        procDataState.caseNumber = res.data['CreateCase_Output']['CaseNo'];
                        procDataState.decision = "Submit";
                        procDataState.userName = ((mastersSelector.hasOwnProperty('auth')?(mastersSelector.auth.hasOwnProperty('userName')?mastersSelector.auth.userName:'system'):'system'));
                        ...
                        if(documentSectionDataRef.current.length>0){
                            ...
                            documentArray = documentArray.filter(x => x.docStatus==='Uploaded');
                            documentArray.forEach(e => {
                                const fileUploadData = new FormData();
                                fileUploadData.append('file', e.fileData);
                                fileUploadData.append('caseNumber', res.data['CreateCase_Output']['CaseNo']);
                                fileUploadData.append('docType', e.documentType);
                                console.log("Inside Add Provider File Upload Data: ",fileUploadData);
                                fileUpDownAxios.post("/uploadFile",fileUploadData).then((response) => {
            ...
            alert("Please select case and populate the data before submittig the case.")
            setButtonDisableFlag(false);
            return;
            }
    ...
    const updateFormData = (values) => {
        ...
        requestBody.agesSeen = apiTestState.agesSeen !== undefined ? apiTestState.agesSeen.value : "";
        requestBody.delegated = values.delegated.value !== undefined ? values.delegated.value : null;
        requestBody.newPatients = apiTestState.newPatients !== undefined ? apiTestState.newPatients.value: "";
        requestBody.placeInDirectory = apiTestState.placeInDirectory !== undefined ? apiTestState.placeInDirectory.value : "";
       // values.states = ((apiTestState.states!==undefined)?apiTestState.states.map(el => el.value).toString():apiTestState.states);
        ...
        const gridKeys = getTableDetails()['providerLinear'].concat(getTableDetails()['gridTables']);
        gridKeys.forEach((k) => {
        ...
        customAxios.post("/generic/update",gridDataRef.current,{headers:{'Authorization':`Bearer ${token}`}})
        .then((res) => {
        ...
        if(tabRef.current === "DashboardView"){
            ...
            return (
                <>
                    ...
                    <DecisionTab lockStatus={(prop.state.lockStatus===undefined)?'N':prop.state.lockStatus}></DecisionTab>
...
const callGenericGet= () => {
    ...
    if(numberOfChecks && numberOfChecks.PROVIDERSTATUS && numberOfChecks.PROVIDERSTATUS=='Terminated')
    {
        alert("Data can't be populated as Provider status is Terminated. Please choose Active cases.")
    }
    else
    {
    let getApiJson= {};
    ...
    {
        getApiJson['tableNames'] =  getTableDetails()['pdmProviderLinear'].concat(getTableDetails()['pdmPayToTable']);
    }
   ...
   if(numberOfChecks.CONTRACTID!=undefined && numberOfChecks.PROVIDERID!=undefined)
    {
    getApiJson['whereClause'] = {'CONTRACTID':numberOfChecks.CONTRACTID, 'PROVIDERID': numberOfChecks.PROVIDERID};
    console.log("getApiJson: ",getApiJson);
    customAxios.post('/generic/get',getApiJson,{headers:{'Authorization':`Bearer ${token}`}}).then((res) => {
        console.log("Generic get api response: ",res);
        ...
        if(apiStat === 0){
            ...
            let linearFieldArray = ['organizationName','firstName', 'middleName','lastName','suffix','caqhId','ssn',
            'medicareId','medicaidId','emailId'];

            for (let field in linearFieldArray) {
                //console.log("field Nidhi02: ",field);
                if ( hasOwnPropertyCaseInsensitive(apiResponse , linearFieldArray[field])) {
                    console.log("field xx03: ",apiResponse,linearFieldArray[field]);
                    apiResponseNew[linearFieldArray[field]] = apiResponse[getKeyCase(apiResponse,linearFieldArray[field])] ;
                  }
            ...
            let linearFieldSelectArray = ['gender','newPatients', 'agesSeen','placeInDirectory','delegated','contractId'];
            for (let field in linearFieldSelectArray) {
                if ( hasOwnPropertyCaseInsensitive(apiResponse , linearFieldSelectArray[field])) {
                    console.log("field xx03: ",apiResponse,linearFieldSelectArray[field]);
                    apiResponseNew[linearFieldSelectArray[field]] = {'label':apiResponse[getKeyCase(apiResponse,linearFieldSelectArray[field])],'value':apiResponse[getKeyCase(apiResponse,linearFieldSelectArray[field])]};
                  }
            ...
            if ( hasOwnPropertyCaseInsensitive(apiResponse , 'DATE_OF_BIRTH#date')) {
                if(typeof apiResponse[getKeyCase(apiResponse, 'DATE_OF_BIRTH#date')] === 'string'){
                    const dob = new Date(apiResponse[getKeyCase(apiResponse, 'DATE_OF_BIRTH#date')]);
             ...
             let gridFieldArray = ['locationName','stateValue', 'zipCode','officeFaxNumber','officePhoneNumber','languages','npi','addressType',
             'address1','address2','city','county','tddPhone','telemedicine'];
             for (let field in gridFieldArray) {
                 //console.log("field Nidhi02: ",field);
                 if ( hasOwnPropertyCaseInsensitive(apiResponse , gridFieldArray[field])) {

                     apiResponseNew[gridFieldArray[field]] = apiResponse[getKeyCase(apiResponse,gridFieldArray[field])] ;
                   }
            ...
            //Newly Added on 07/28/2023
            let gridFieldSelectArray = ['electronicHealthRecord','publicTransportation', 'handicapAccess','tddHearing'];
            for (let field in gridFieldSelectArray) {
                if ( hasOwnPropertyCaseInsensitive(apiResponse , gridFieldSelectArray[field])) {
                    if ( apiResponse[getKeyCase(apiResponse, gridFieldSelectArray[field])] === 'Y')
                    {
                        apiResponseNew[gridFieldSelectArray[field]] = {'label':'Yes','value':apiResponse[getKeyCase(apiResponse,gridFieldSelectArray[field])]};}
            else if ( apiResponse[getKeyCase(apiResponse, gridFieldSelectArray[field])] === 'N')
                    { apiResponseNew[gridFieldSelectArray[field]] = {'label':'No','value':apiResponse[getKeyCase(apiResponse,gridFieldSelectArray[field])]};}
             }

            }
                        apiResponseNew.rowNumber=apiResponseArray.length +1;
                        apiResponseArray.push(apiResponseNew);
                        console.log("locationTableRowsData apiResponse: ", apiResponseNew);
                    });
             ...
             let gridFieldArray = ['taxId','locationName', 'address1','address2','city','county','stateValue','zipCode','payToNpi'];
             for (let field in gridFieldArray) {
                 console.log("field Nidhi02: ",field);
                 if ( hasOwnPropertyCaseInsensitive(apiResponse , gridFieldArray[field])) {
                     ...
                     apiResponseNew[gridFieldArray[field]] = apiResponse[getKeyCase(apiResponse,gridFieldArray[field])] ;
                        ...
                        apiResponseNew.rowNumber=apiResponseArray.length +1;
                        apiResponseArray.push(apiResponseNew);
                        console.log("payToTableRowsData apiResponse: ", apiResponseNew);
                    });
...
//Till here
    ...
    const populateForm = () => {
        ...
        console.log("selectValues.stateOptionsLinear:", selectValues.stateOptionsLinear);
        return (
                <div className="col-xs-12">
                    <div className="accordion ProviderDemoLabel" id="accordionPanelsStayOpenExample">
					    <Formik
					...
					>
					    {props => {
                            ...
                            <div className="accordion-item">
                                ...
                                <button className="accordion-button accordionButtonStyle" type="button" data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapseInstructions" aria-expanded="true" aria-controls="panelsStayOpen-collapseOne">
                                Instructions
                                </button>
                                </h2>
                                <div id="panelsStayOpen-collapseInstructions" className="accordion-collapse collapse show" aria-labelledby="panelsStayOpen-Instructions">
                                    <div className="accordion-body">
                                        <div className="row">
                                            <div className="col-xs-12">
                                                <label id="instructionHeading" className='instructionHeading'>To modify Provider Address, please provide Organization Name and select Provider Address Modification Switch. Review all information to confirm the accuracy.</label>
                           ...
                           {(prop.state.formView === 'HomeView') ||
                            ...
                            <div className="accordion-item">
                                <h2 className="accordion-header" id="panelsStayOpen-Address">
                                <button className="accordion-button accordionButtonStyle" type="button" data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapseAddress" aria-expanded="true" aria-controls="panelsStayOpen-collapseOne">
                                Provider/ PayTo Address Modification
                                </button>
                                </h2>
                                <div id="panelsStayOpen-collapseAddress" className="accordion-collapse collapse show" aria-labelledby="panelsStayOpen-Address">
                                    <div className="accordion-body">

                                    <div className="row">

                                                  <div className="col-xs-6 col-md-4">
                                                     <label htmlFor="medicaidSwitch">Provider Address Modification</label>
                                                       <br/>
                                             <Switch id="addModificationSwitch" name="addModification"
                                            //  onChange={isChecked => {
                                            //     if(caseUnlockState !== -1){
                                            //         setCaseUnlockState(-1);
                                            //     }
                                            ...
                                            checked={(apiTestState.AddModification!==undefined)?apiTestState.AddModification:false}
                                            uncheckedIcon={false}
                                            checkedIcon={false}
                                            disabled={(tabRef.current==='DashboardView'&&(prop.state.lockStatus!==undefined && prop.state.lockStatus==='Y')?true:false)}
                                            />
                                          ...
                                          <div className="col-xs-6 col-md-4">
                                         <label htmlFor="medicaidSwitch">Provider PayTo Modification</label>
                                        <br/>
                                             <Switch
                                            ...
                                            //     });
                                            // }}
                                            checked={(apiTestState.PayToModification!==undefined)?apiTestState.PayToModification:false}
                                            uncheckedIcon={false}
                                            checkedIcon={false}
                                            disabled={(tabRef.current==='DashboardView'&&(prop.state.lockStatus!==undefined && prop.state.lockStatus==='Y')?true:false)}
                                            />
    ...
    {(apiTestState.AddModification==true || apiTestState.PayToModification==true)&& (prop.state.formView === "DashboardHomeView") ?
                            ...
                            <div className="col-xs-6 col-md-4">
                              <div className="form-floating">
                                <Select
                                  ...
                                  placeholder="Please choose Organization Name"
                                // isClearable
                                  isSearchable={
...
{(apiTestState.AddModification==true || apiTestState.PayToModification==true)&& ((prop.state.formView === 'HomeView') || (prop.state.formView === "DashboardHomeView")) ?
                                             <div className="col-xs-6">
                                                    <TableComponent columnName={providerColumnNames}
                                        ...
                                        {(apiTestState.AddModification==true || apiTestState.PayToModification==true)?
                                                  <div className="col-xs-6 col-md-4" style={{textAlign:"right", marginLeft:"270px"}}>
                                                  <button type="button" className="btn btn-outline-primary btnStyle" onClick={() => callGenericGet()}
                            ...
                            <div className="accordion-item" id='providerInformation'>
                                ...
                                <button className="accordion-button accordionButtonStyle" type="button" data-bs-toggle="collapse" data-bs-target="#panelsStayOpen-collapseProvider" aria-expanded="true" aria-controls="panelsStayOpen-collapseOne">
                                    ...
                                    {tabRef.current === "DashboardView"?
                                        ...
                                        <div className="row my-2">
                                            <div className="col-xs-6 col-md-4">
													<Field name="organizationName">
														...
														<div className="form-floating">
                                                            ...
                                                            onChange={(e) => formikFieldsOnChange(e,setFieldValue,field)} disabled/>
															<label htmlFor="floatingInputGrid">Organization Name</label>
                                            ...
                                            <div className="col-xs-6 col-md-4">
												<Field name="firstName">
													...
													<div className="form-floating">
														<input id="firstName" maxLength="100" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":field.value?"is-valid":""}`} placeholder="John" {...field}
                                                        onChange={(e) => formikFieldsOnChange(e,setFieldValue,field)} disabled/>
                                            ...
                                            <div className="col-xs-6 col-md-4">
												<Field name="middleName">
													...
													<div className="form-floating">
														<input maxLength="100" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":(field.value || field.value===null)?"is-valid":""}`} placeholder="John" {...field} disabled/>
                                        ...
                                        <div className="row my-2">
                                            ...
                                            <div className="col-xs-6 col-md-4">
												<Field name="suffix">
													...
													<div className="form-floating">
														<input maxLength="10" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":field.value?"is-valid":""}`} placeholder="John" {...field} disabled/>
                                        ...
                                        <div className="row my-2">
                                            ...
                                            <div className="col-xs-6 col-md-4">
												<Field name="caqhId" innerRef={caqhId}>
													...
													<div className="form-floating">
                                                        ...
                                                        disabled />
														<label htmlFor="floatingInputGrid">CAQH ID</label>
														{meta.touched && meta.error && (
															<div className="invalid-feedback" style={{display:'block'}}>{meta.error}</div>
														)}
													</div>
                                            ...
                                            <div className="col-xs-6 col-md-4">
												<Field name="caqhNpiId">
													...
													<div className="form-floating">
														<input maxLength="10" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":field.value?"is-valid":""}`} placeholder="John"
                                                        //oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
                                                        {...field}
                                                        ...
                                                        //ref = {caqhNpiIdRef}
                                                        disabled />
														<label htmlFor="floatingInputGrid">NPI ID</label>
														{meta.touched && meta.error && (
															<div className="invalid-feedback" style={{display:'block'}}>{meta.error}</div>
														)}
													</div>
                                            ...
                                            <div className="col-xs-6 col-md-4">
                                              ...
                                              />
												...
												<Field name="medicareId">
													...
													<div className="form-floating">
														<input maxLength="10" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":field.value?"is-valid":""}`} placeholder="John" {...field} disabled/>
														<label htmlFor="floatingInputGrid">Medicare Id</label>
														{meta.touched && meta.error && (
															<div className="invalid-feedback" style={{display:'block'}}>{meta.error}</div>
														)}
													</div>
                                        ...
                                        <div className="col-xs-6 col-md-4">
											<Field name="medicaidId">
												...
												<div className="form-floating">
													<input maxLength="15" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":field.value?"is-valid":""}`} placeholder="John" {...field} disabled/>
													<label htmlFor="floatingInputGrid">Medicaid Id</label>
													{meta.touched && meta.error && (
														<div className="invalid-feedback" style={{display:'block'}}>{meta.error}</div>
													)}
												</div>
                                          ...
                                          <div className="row my-2">
                                                    <div className="col-xs-6 col-md-4">
                                                            <Field name="emailId">
                                                                ...
                                                                <div className="form-floating">
                                                                    <input maxLength="50" type="text" className={`form-control ${(meta.touched && meta.error)?" is-invalid":field.value?"is-valid":""}`} placeholder="John" {...field} disabled/>
                                ...
                                <div id="panelsStayOpen-collapseAddress" className="accordion-collapse collapse show" aria-labelledby="panelsStayOpen-Address">
                                    <div className="accordion-body">
                                        <div className="row">
                                            ...
                                            <LocationTable locationTableRowsData={locationTableRowsData} addTableRows={addTableRows}
                                                deleteTableRows={deleteTableRows} handleGridSelectChange={handleGridSelectChange} handleGridFieldChange={handleGridFieldChange} gridRowsFinalSubmit={gridRowsFinalSubmit}
                                                selectJson={selectValues} calledFormName={ProviderDemo.validate}
                                                ...
                                                fetchAutoPopulate={fetchAutoPopulate}></LocationTable>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                         ...
                         {apiTestState.PayToModification==true?
                            ...
                            <div className="col-xs-6 col-md-12">
                                ...
                                modifyValidatedAddressPayToRow={modifyValidatedAddressPayToRow}
                                lockStatus={
                                  prop.state !== null &&
                                  prop.state.lockStatus !== undefined
                                ...
                                fetchAutoPopulate={fetchAutoPopulate}></PayToTable>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            :<div></div>}
                                ...
                                <div style={{display:'flex', alignItems: 'center', justifyContent:'center', marginTop: 30, marginBottom: 20}}>
                                    <button id="mainFormSubmit" type="button" className="providerPageButton button" onClick={event => {handleSubmit(event)}}
                                    style={{display:'none'}}>{isSubmitting?"Saving":"Save"}</button>
                                </div>
                            </fieldset>
							</form>
							)
    ...
    const addRoster = (caqhId)=>{
        ...
        const payload = {
            ...
            setLoading(true);
        customAxios.post('caqh/addToRoster', payload, {headers:{'Authorization': `Bearer ${token}`}}).then(res => {
    ...
    return( loadForm &&
        <>
            ...
            />
            <AlertModal variant={alertModalShow.variant} message={alertModalShow.message} show={alertModalShow.show} alertShowHide={alertShowHide}/>
            <div className = "AddProvider backgroundColor" style={{minHeight:"100vh"}}>
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-xs-6" style={{textAlign:"center"}}>
                            <br/>
                            <button type="button" className="btn btn-outline-primary btnStyle" onClick={event => navigateHome(event)} style={{float:"left",marginLeft:"10px"}}>Go To Home</button>
                            {/* <button type="submit" className="providerPageButton button" onClick={event => {handleSubmit()}}>{isSubmitting?"Saving":"Save"}</button> */}
                            <label id="tileFormLabel" className='HeadingStyle'>Provider Modification</label>
                            {(tabRef.current==='DashboardView' ) &&
                ...
                <FooterComponent/>
            <Modal
                    onHide={()=>{setCaqhModal({id:null, header: null, body: null, show:false});}}
                    backdrop="static"
                    keyboard={false}
                    show={caqhModal.show}
                ...
                <Modal
                    onHide={()=>{setCaqhGenericModal({body: null, header:null, state:false});}}
                    backdrop="static"
                    keyboard={false}
