...
export default function useCallApi() {
    ...
    const getLinkingData = async (token,type,userName,cname) => {
        ...
        console.log("Inside useCallApi getLinkingData Data after promise resolve ",res);
        if(res !== undefined){
            if(res.data.CallProcedure_Output.Status === 0){
                respData = [...res.data.CallProcedure_Output.data];
                //respData = JSON.parse(respData)
                console.log('Get data Response for getProvContLinkData 2: ',respData[0])
              }
        }
        return respData;
    }
    ...
    const executeApi = async(token, getApiJson) => {
        let respData = await axios.post('/generic/callProcedure',getApiJson,{headers:{'Authorization':`Bearer ${token}`}})
          console.log("Inside useCallApi executeApi Data before promise resolve ",respData);
    ...
    const downloadFile = (index, documentData) => {
        ...
        const caseId = (documentData[index]?.caseNumber===undefined || documentData[index]?.caseNumber === null)?0:Number(documentData[index]?.caseNumber);
        printConsole('Inside useCallApi hook downloadFile caseNumber: ',caseId);
        const documentType = (documentData[index]?.documentType===undefined)?'':documentData[index]?.documentType;
        ...
        // fileUpDownAxios.get("/downloadFile/"+caseId+'/'+documentType+'/'+documentName,fileData,{responseType: 'blob'}).then((response) => {
            fileUpDownAxios.post("/downloadFile",fileData,{responseType: 'blob'}).then((response) => {
                ...
                if(downloadFilePath !== ''){
                    docName = downloadFilePath.substring(downloadFilePath.lastIndexOf("/") + 1);
                }
                else{
                ...
                const filename =  appendName + '_' + caseId +  (docName.substring(docName.lastIndexOf('.'),docName.length));
