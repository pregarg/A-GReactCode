export default function useFormikValidation() {
  const checkErrorsAndFocusOnFields = (
    errors,
    setFieldTouched,
    handleSubmit,
    evnt,
    ...
    if (errorKeys.length === 0) {
      ...
      let divElems = document.querySelectorAll(
        '[id*="justify-tab-example-tabpane"]',
      );
      ...
      if (divElems.length > 0) {
        ...
        for (var i = 0; i < divElems.length; i++) {
          inputField = divElems[i].querySelector(
            `input[name="${errorKeys[0]}"]`,
          );
        ...
        if (containsDiv !== "" && inputField !== "") {
          ...
          const ariaLabelledByValue =
            containsDiv.getAttribute("aria-labelledby");
    ...
    if (decision !== "Discard") {
      ...
      if (delegated === null || delegated === undefined) {
        ...
        if (delegated.hasOwnProperty("value")) {
          return delegated.value !== undefined
            ? delegated.value.toLowerCase() === "yes"
            : false;
        } else {
          return delegated !== undefined
            ? delegated.toLowerCase() === "yes"
            : false;
        }
      }
    } else {
