...
export default function useUpdateDecision() {
  ...
  const updateDecision = (location, saveType, transactionType) => {
    ...
    procInput.DecisionNotes =
      location.state.decisionNotes != undefined
        ? location.state.decisionNotes.trim()
        : "";
  ...
  const submitCase = (prop, navigateHome) => {
    ...
    if (prop.state.decision !== undefined && prop.state.decision !== "") {
      ...
      axios
        .put("/updateCaseDecision", updateInputs, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => {
          ...
          if (res.status === 200) {
            ...
            procInput.flowId = prop.state.flowId;
            procInput.decisionReason = prop.state.decisionReason
              ? prop.state.decisionReason
              : "";
  ...
  const validateGridData = (gridJson) => {
    ...
    gridJson.forEach((elem) => {
      const stat =
        elem?.status !== undefined && elem?.status !== null
          ? elem.status.value !== undefined
            ? elem.status.value
            : elem?.status
          : elem?.status;
  ...
  const setContractIdDropDown = (mainTableData, secondData) => {
    ...
    if (secondData.length > 0) {
      secondData.forEach((elem) => {
        if (!mainTableDataArr.includes(elem.contractid)) {
          if (
            elem.contractid !== null &&
            elem.contractid !== undefined &&
            elem.contractid !== ""
          ) {
    ...
    printConsole(
      "Inside setContractIdDropDown mainTableDataArr after filtering: ",
      mainTableDataArr,
    );
    let contArr = [];
  ...
  const masterExclusionListSelector = useSelector(
    (state) => state?.masterExclusionList,
  );
  ...
  const getNPIFromMaster = (caqhNpiId, decision, callProc) => {
    ...
    if (decision?.toLowerCase() !== "discard" && callProc !== "notCallProc") {
      if (
        masterExclusionListSelector[0] !== undefined &&
        masterExclusionListSelector[0] !== null
      ) {
        let NPIres = masterExclusionListSelector[0].find(
          (el) => el.NPI === caqhNpiId,
        );
        if (NPIres !== undefined) {
          responseNPI = true;
  ...
  const getCountyFromMaster = (DataState, Zip) => {
    ...
    let responseCounty = "";
    if (
      masterCountySelector[0] !== undefined &&
      masterCountySelector[0] !== null
    ) {
      masterCountySelector[0].forEach((elem) => {
        if (
          elem.StateId === DataState.trim() &&
          elem.ZipCodes.includes(Zip.trim())
        ) {
    ...
    try {
      ...
      keys1.filter((dataValue) => {
        if (Json2.hasOwnProperty(dataValue)) {
          ...
          // else{
          if (
            String(Json1[dataValue]).toLowerCase().trim() !==
            String(Json2[dataValue]).toLowerCase().trim()
          ) {
  ...
  function areAllLocationNameSame(locations, dba) {
    ...
    const firstLocationName =
      locations[0]?.locationName?.value ?? locations[0]?.locationName;
    for (let i = 1; i < locations.length; i++) {
      const templocation =
        locations[i]?.locationName?.value ?? locations[i]?.locationName;
