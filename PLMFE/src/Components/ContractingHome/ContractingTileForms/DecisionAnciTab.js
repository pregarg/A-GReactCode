...
export default function DecisionAnciTab() {
  ...
  const uploadFile = (index) => {
    ...
    if (fileState.selectedFile !== null) {
      ...
      axios
        .post("http://localhost:8081/dashboard/api/uploadFile", fileData)
        .then((res) => {
  ...
  const downloadFile = (index) => {
    ...
    const documentType = documentData[index].documentName.value;
    const apiUrl =
      "http://localhost:8081/dashboard/api/downloadFile/" +
      caseId +
      "/" +
      documentType;
          ...
          <>
            <tr key={index}>
              <td className="tableData">
                {"documentName" in data && data.documentName.value !== undefined
